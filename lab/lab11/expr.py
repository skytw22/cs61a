...
class CallExpr(Expr):
    ...
    def eval(self, env):
        ...
        for operand in self.operands:
            values.append(operand.eval(env))
...
class LambdaFunction(Value):
    ...
    def apply(self, arguments):
        ...
        env_copy = self.parent.copy() # Make a copy of the parent environment
        for (k, v) in zip(self.parameters, arguments):
            env_copy.update({k: v}) # Update the copy with parameters and arguments passed in
        return self.body.eval(env_copy) # Evaluate the body using the newly created environment
