...
def eval_all(expressions, env):
    ...
    expr = expressions
    eval_expr = scheme_eval(expr.first, env)
...
class Frame(object):
    ...
    def lookup(self, symbol):
        ...
        parent = self.parent
        while parent != None:
            if parent.bindings.get(symbol) != None:
                return parent.bindings.get(symbol)
...
def do_and_form(expressions, env):
    ...
    while is_true_primitive(value) and expressions.rest is not nil:
...
def do_or_form(expressions, env):
    ...
    if is_true_primitive(value):
            return value
    while is_false_primitive(value) and expressions.rest is not nil:
        expressions = expressions.rest
        value = scheme_eval(expressions.first, env)
        if is_true_primitive(value):
            return value
