...
def swine_align(player_score, opponent_score):
    ...
    if opponent_score == 0:
        return False
    d = 10
    while d <= player_score or d <= opponent_score:
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    if who == 0:
        def say(score0, score1):
            score_gain = score0 - last_score
            if score_gain > running_high:
                print(str(score_gain) + ' point(s)!' + ' The most yet for Player 0')
                return announce_highest(0, score0, score_gain)
            return announce_highest(0, score0, running_high)
        return say
    else:
        def say(score0, score1):
            score_gain = score1 - last_score
            if score_gain > running_high:
                print(str(score_gain) + ' point(s)!' + ' The most yet for Player 1')
                return announce_highest(1, score1, score_gain)
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    max_num_rolls = 1
    while num_rolls <= 10:
        score = make_averaged(roll_dice, trials_count)(num_rolls, dice)
        if score > max_score:
            max_score = score
