...
def about(topic):
    ...
    # BEGIN PROBLEM 2
    def func(paragraph):
        removed_punc_para = remove_punctuation(paragraph)
        lower_case_para = lower(removed_punc_para)
        list = split(lower_case_para)
        for item in topic:
            if item in list:
                return True
        return False
...
def wpm(typed, elapsed):
    ...
    # BEGIN PROBLEM 4
    num_of_words = len(typed) / 5.0
    wpm = num_of_words / elapsed * 60.0
    return wpm
    # END PROBLEM 4
...
def autocorrect(user_word, valid_words, diff_function, limit):
    ...
    for word in valid_words:
        ...
        list.append(diff)
    min_diff = min(list)
    min_diff_index = list.index(min_diff)
    if min_diff > limit:
        return user_word
    else:
...
def pawssible_patches(start, goal, limit): #tree recursion
...
def time_per_word(times_per_player, words):
    ...
    for player in times_per_player:
        times.append([player[i+1] - player[i] for i in range(0, len(words))])
...
def fastest_words(game):
    ...
    for word_index in word_indices:
        min = time(game, 0, word_index)
        player_num = 0
        for player in player_indices:
            if time(game, player, word_index) < min:
                player_num = player
                min = time(game, player, word_index)
        word = word_at(game, word_index)
        min_list[player_num].append(word)
    return min_list
